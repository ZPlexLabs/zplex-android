...
class AboutActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        save_show.setOnClickListener {
            if (file != null) {
                viewModel.saveFile(file)
                Snackbar.make(root, "$nameShow saved successfully", Snackbar.LENGTH_SHORT).show()
            } else {
                Snackbar.make(root, "$nameShow not saved", Snackbar.LENGTH_SHORT).show()
            }
        }
                        ...
                        resource?.let {
                            Palette.from(resource).generate { p: Palette? ->
                                p?.let {
                                    ...
                                    save_show!!.iconTint =
                                        ColorStateList.valueOf(getContrastColor(colorPrimary))
                                    save_show!!.setTextColor(getContrastColor(colorPrimary))
                                    save_show!!.strokeColor = ColorStateList.valueOf(accent)
