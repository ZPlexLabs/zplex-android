...
public class CustomStyledPlayerView extends StyledPlayerView implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener {
    ...
    private final float IGNORE_BORDER = Utils.Companion.dpToPx(24);
    private final float SCROLL_STEP = Utils.Companion.dpToPx(16);
    private final float SCROLL_STEP_SEEK = Utils.Companion.dpToPx(8);
    ...
    public CustomStyledPlayerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        exoErrorMessage.setOnClickListener(v -> {
            if (PlayerActivity.locked) {
                ...
                Utils.Companion.showText(CustomStyledPlayerView.this, "", MESSAGE_TIMEOUT_LONG);
                setIconLock(false);
            }
        });
    ...
    public boolean tap() {
        if (PlayerActivity.locked) {
            Utils.Companion.showText(this, "", MESSAGE_TIMEOUT_LONG);
            setIconLock(true);
            return true;
        }
    ...
    public boolean onScroll(MotionEvent motionEvent, MotionEvent motionEvent1, float distanceX, float distanceY) {
        ...
        if (gestureOrientation == Orientation.HORIZONTAL || gestureOrientation == Orientation.UNKNOWN) {
            ...
            if (Math.abs(gestureScrollX) > SCROLL_STEP || (gestureOrientation == Orientation.HORIZONTAL && Math.abs(gestureScrollX) > SCROLL_STEP_SEEK)) {
                ...
                long position = 0;
                float distanceDiff = Math.max(0.5f, Math.min(Math.abs(Utils.Companion.pxToDp(distanceX) / 4), 10.f));
                ...
                if (PlayerActivity.haveMedia) {
                    ...
                    String message = Utils.Companion.formatMillisSign(seekChange);
                    if (!isControllerFullyVisible()) {
                        message += "\n" + Utils.Companion.formatMillis(position);
                    }
    ...
    public boolean onScale(ScaleGestureDetector scaleGestureDetector) {
        ...
        if (canScale) {
            ...
            mScaleFactor = Utils.Companion.normalizeScaleFactor(mScaleFactor);
