...
class AboutFragment : Fragment(R.layout.fragment_about) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        argsModel.args.observe(viewLifecycleOwner, { arg ->
            ...
            aboutViewModel.getShow(arg.file.id).observe(viewLifecycleOwner, { isSaved ->
                Log.d("getShow", "isSaved=$isSaved")

                binding.mbSave.apply {
                    icon = ContextCompat.getDrawable(
                        context,
                        if (isSaved) R.drawable.ic_favorite_24 else R.drawable.ic_favorite_border_24
                    )
                    setOnClickListener {
                        if (isSaved) aboutViewModel.deleteShow(
                            arg.file
                        ) else aboutViewModel.saveShow(arg.file)

                        val status = if (isSaved) "removed" else "saved"
                        Snackbar.make(
                            binding.root,
                            "${arg.name} $status successfully",
